...
export async function POST(
  ...
  console.log("ACCEPT Received paramsPromise:", paramsPromise);
  try {
    const params = await paramsPromise;
    console.log("ACCEPT Resolved params (after await):", params);

    const paramParseResult = SuggestionIdParamSchema.safeParse(params);
    if (!paramParseResult.success) {
      console.error("ACCEPT Error parsing suggestionId params:", paramParseResult.error);
    ...
    if (!bodyParseResult.success) {
      console.error('ACCEPT AcceptSuggestion validation errors:', bodyParseResult.error);
      return NextResponse.json(
        { error: 'Invalid request body', details: bodyParseResult.error.format() },
        { status: 400 }
      );
    }
    ...
    console.error("ACCEPT Error in POST /api/flashcards-suggestions/[suggestionId]/accept:", err);
