...
import { createShareCommandSchema } from "@/features/schemas/flashcardsSetSchemas";
...
export async function POST(
  ...
  try {
    ...
    const bodyValidation = createShareCommandSchema.safeParse(body);
    ...
    const newShare = await service.share(user.id, setIdValidation.data, bodyValidation.data);
...
export async function GET(
  ...
  try {
    ...
    const service = new FlashcardsSetService(supabase);
    const shares = await service.listShares(user.id, setIdValidation.data);
