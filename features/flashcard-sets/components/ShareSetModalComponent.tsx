...
import { useGetCollaborators, useInviteCollaborator, useRevokeShare } from "@/features/flashcard-sets/api/useShareSet";
...
export const ShareSetModalComponent: React.FC<{ onCancel: () => void }> = ({ onCancel }) => {
  ...
  const { data: collaborators = [], isLoading: isLoadingCollaborators } = useGetCollaborators(setId);
  const { mutate: invite, isPending: isInviting } = useInviteCollaborator(setId);
  ...
  useEffect(() => {
    const searchUser = async () => {
      ...
      try {
        const response = await fetch(`/api/users/find-by-email?email=${encodeURIComponent(debouncedEmail)}`);
  ...
  const handleAction = () => {
    ...
    if (role === 'viewer') {
      ...
      cloneForUser({
        ...
        onSuccess: () => {
          toast.success(`Kopia zestawu została wysłana do ${foundUser.email}`);
          setEmail("");
          setFoundUser(null);
        },
        onError: (e) => toast.error(`Błąd: ${e.message}`),
  ...
  const isButtonDisabled = isActionPending || !foundUser || (role === 'viewer' && !!collaborators.find(c => c.userId === foundUser?.id));
      ...
      <div className="flex gap-2 items-start">
        <div className="flex-grow">
          ...
          <div className="text-xs text-muted-foreground h-4 mt-1">
            ...
            {foundUser && <p className="text-green-600">Znaleziono: {foundUser.email}</p>}
            {foundUser && role === 'viewer' && collaborators.find(c => c.userId === foundUser.id) && <p className="text-yellow-600">Ten użytkownik ma już dostęp.</p>}
          </div>
        </div>
        ...
        <Button onClick={handleAction} disabled={isButtonDisabled} className="w-32">
          {isActionPending ? "Przetwarzanie..." : buttonText}
        </Button>
      </div>

      <p className="text-sm font-medium pt-4 border-t">Udostępniono w trybie "Podgląd":</p>
      <ScrollArea className="h-48 pr-2 border rounded-md">
        {isLoadingCollaborators ? (
          <p className="p-2">Ładowanie...</p>
        ) : (
          collaborators.map((c) => (
            <div key={c.id} className="flex items-center justify-between px-2 py-1 border-b text-sm hover:bg-muted/50">
              <span className="truncate" title={c.userId}>{c.userId}</span>
              <div className="flex items-center gap-2">
                <Badge variant="secondary">{c.role}</Badge>
                <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => handleRevoke(c.id)} disabled={isRevoking}>
        ...
        {collaborators.length === 0 && !isLoadingCollaborators && (
          <p className="p-2 text-muted-foreground">Zestaw nie jest nikomu udostępniony.</p>
        )}
      </ScrollArea>
