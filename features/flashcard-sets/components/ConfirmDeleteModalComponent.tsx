...
export function ConfirmDeleteModalComponent({ isOpen, onClose, set }: ConfirmDeleteModalComponentProps) {
  const { deleteSet, isMutating, error: apiError } = useFlashcardSetsStore();
  ...
  const handleDelete = async () => {
    ...
    // Błędy API są zarządzane w store i mogą być wyświetlane globalnie
    // lub można dodać wyświetlanie apiError tutaj, jeśli jest taka potrzeba.
  };
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader className="space-y-4">
          ...
          <DialogTitle className="text-center text-lg font-semibold leading-6 text-gray-900">Potwierdź usunięcie zestawu</DialogTitle>
