...
test.describe('Authentication', () => {
  ...
  test('SCN_AUTH_001: should register a new user successfully', async ({ page }) => {
  ...
  test('SCN_AUTH_002: should show error when registering with existing email', async ({ page }) => {
    await page.goto('/sign-up');
    ...
    await expect(page.getByText(/email już istnieje|konto już istnieje/i)).toBeVisible();
  });

  // SCN_AUTH_003: Registration with mismatched passwords
  test('SCN_AUTH_003: should show error when passwords do not match', async ({ page }) => {
    await page.goto('/sign-up');
    await page.getByLabel('Email').fill(`test${Date.now()}@example.com`);
    await page.getByLabel(/^Hasło/).fill('Test@1234');
    await page.getByLabel(/Potwierdź hasło|Powtórz hasło/).fill('DifferentPassword');
    await page.getByRole('button', { name: /Zarejestruj/i }).click();

    // Verify error message
    await expect(page.getByText(/hasła nie pasują|hasła nie są zgodne/i)).toBeVisible();
  });

  // SCN_AUTH_004: Successful login
  test('SCN_AUTH_004: should login successfully with valid credentials', async ({ page }) => {
  ...
  test('SCN_AUTH_005: should show error with invalid credentials', async ({ page }) => {
    await page.goto('/sign-in');
    await page.getByLabel('Email').fill(E2E_EMAIL);
    ...
    // Verify error message
    await expect(page.getByText(/nieprawidłowy email lub hasło|błędne dane logowania/i)).toBeVisible();
  });
  ...
  test('SCN_AUTH_007: should redirect to login when accessing protected page without login', async ({ page }) => {
  ...
  test('SCN_AUTH_008: should access protected page after login', async ({ page }) => {
    // Login first
    await page.goto('/sign-in');
    await page.getByLabel('Email').fill(E2E_EMAIL);
    await page.getByLabel('Hasło').fill(E2E_PASSWORD);
